from Tinder.config.config_app import DB_USER, DB_NAME, STANDARD_SEARCH_OFFSET
    ...
    with pg.connect(dbname=DB_NAME, user=DB_USER) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS last_run_state(
                        ...
                        last_desired_age_from INTEGER,
                        last_desired_age_to INTEGER,
...
def init_user_in_last_run_state(user_vk_id: StrOrInt) -> None:
    ...
    with pg.connect(dbname=DB_NAME, user=DB_USER) as conn:
        with conn.cursor() as cur:
            cur.execute("""INSERT INTO last_run_state (vk_id, current_offset, last_desired_age_from, 
                            last_desired_age_to) VALUES (%s, %s, %s, %s) ON CONFLICT (vk_id) DO NOTHING """,
